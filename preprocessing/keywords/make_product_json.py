...
JSON은 리스트를 키로 직접 쓸 수 없어, 키는 리스트를 직렬화한 문자열로 저장.
...
def format_product_block(key: str, keywords: list[str]) -> str:
    """예시 형태: {["상품명", 가격, 용량]: ["키워드1", "키워드2"]}"""
    keywords_text = json.dumps(keywords, ensure_ascii=False)
    return f"{{{key}: {keywords_text}}}"
...
def build_products(df: pd.DataFrame) -> list[dict[str, list[str]]]:
    ...
    for name, group in df.groupby(COL_NAME, sort=False):
        ...
        # JSON 키는 문자열만 허용하므로 리스트 표현을 문자열화해서 사용
        ...
        result.append({key: list(seen.keys())})
...
def main() -> None:
    ...
    for i, item in enumerate(data):
        if i >= 3:
            break
        key, keywords = next(iter(item.items()))
        print(format_product_block(key, keywords))
        print()
