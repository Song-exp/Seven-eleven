...
def predict_proba(
    ...
    """product 성공 확률 (P,) 반환."""
    model.eval()
    logits = model(edge_index_dict, has_promo=has_promo, insta_m30=insta_m30,
                   edge_attr_dict=edge_attr_dict, timeslot_ratio=timeslot_ratio)
...
def evaluate_mask(
    ...
    prob = predict_proba(model, edge_index_dict, has_promo, edge_attr_dict, insta_m30, timeslot_ratio)
    yt = y[mask].cpu().numpy()
