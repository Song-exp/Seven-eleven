...
def train(config_path: str = "configs/train_config.yaml", save_dir: str = "checkpoints"):
    ...
    ts_ratio = (data["product"].timeslot_ratio.to(device)
                if hasattr(data["product"], "timeslot_ratio") else None)
    ...
    use_ts = cfg["node_feat"].get("use_timeslot_features", False)
    model.use_timeslot_features = use_ts and ts_ratio is not None
    ...
    for epoch in range(1, t["epochs"] + 1):
        ...
        if opt_a:
            opt_a.zero_grad()
        logits = model(edge_index_dict, has_promo=has_promo, insta_m30=insta_m30, edge_attr_dict=edge_attr_dict, timeslot_ratio=ts_ratio)
        ...
        val_m = evaluate_mask(model, edge_index_dict, has_promo, y, val_mask, edge_attr_dict, insta_m30=insta_m30, timeslot_ratio=ts_ratio)
    ...
    test_m = evaluate_mask(model, edge_index_dict, has_promo, y, test_mask, insta_m30=insta_m30, timeslot_ratio=ts_ratio)
