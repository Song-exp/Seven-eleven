...
class HINGNN(nn.Module):
    def __init__(
        ...
        self.use_timeslot_features = False  # trainer에서 동적으로 활성화
        self.ts_feat_lin = nn.Linear(5, hidden_dim)  # [morning, lunch, snack, dinner, night]
    ...
    def _init_product(
        ...
        timeslot_ratio: torch.Tensor = None,
    ) -> torch.Tensor:
        # Output: (P, d)
        ...
        if self.use_timeslot_features and timeslot_ratio is not None:
            acc = acc + self.ts_feat_lin(timeslot_ratio.float())  # (P, d)
        return acc
    ...
    def forward(
        ...
        x_dict["product"] = self._init_product(
            edge_index_dict, has_promo, insta_m30, timeslot_ratio
        )  # (P, d)
