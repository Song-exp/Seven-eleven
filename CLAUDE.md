...
* 탐색적 데이터 분석(EDA) 및 시각화 작업: `eda/.claude-rules.md`
* 데이터 전처리 및 그래프 생성 작업: `src/data_builder/.claude-rules.md`
* GNN 모델 아키텍처 설계 및 수정: `src/models/.claude-rules.md`
* 학습 루프 및 최적화 작업: `src/train/.claude-rules.md`
...
## 7. 세븐일레븐 도메인 데이터 스키마
세션 시작 시 아래 스키마 문서를 참조하십시오. 브릿지·POS·인스타 3개 도메인 테이블의 구조, 외래키, join 주의사항이 정의되어 있습니다.

@docs/data_schema.md

## 10. 실험 기록 규칙 (Experiment Logging)
실험(`run_experiment`)이 완료되어 결과가 확정될 때마다 반드시 아래 파일을 업데이트해야 합니다.

**`docs/experiments_master_log.md`**
* 역할: 프로젝트의 모든 실험 구성(config)과 성능 지표(PR-AUC, ROC-AUC, F1, val-test gap 등)를 한 곳에서 비교하는 마스터 실험 대장.
* 테이블은 **Test PR-AUC 기준 내림차순** 정렬 유지.
* 섹션 2의 인사이트 요약도 새 발견이 있을 때 함께 갱신.
* 실험 결과를 보고한 후, 해당 문서 업데이트를 빠뜨리지 말 것.

## 9. GNN 모델 아키텍처 선행연구
GNN 모델 설계·구현·수정 시 반드시 아래 문서를 먼저 참조하십시오.
KGAT·HGT·DiffMG 3편의 적용 논리 및 각 논문이 해결하는 문제가 정의되어 있습니다.

@docs/model_architecture.md

## 8. 데이터 분석 전 스키마 선행 확인 (Schema-First Rule)
