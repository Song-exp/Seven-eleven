...
EDGELIST_PARQUET    = os.path.join(DATA_DIR, "final_edgelist_with_trend.parquet")
...
def load_graph() -> nx.Graph:
    ...
    없으면                     → 모드 A (parquet으로부터 빌드)
      - final_edgelist_with_trend.parquet 존재 시: 엣지를 해당 파일에서 직접 로드 (weight 반영)
      - 없으면: product/ip parquet에서 엣지 재구성 (weight=1 고정)
    """
...
def check_trend_node(G: nx.Graph, trend_text: str) -> Optional[str]:
    ...
    for node_id, data in G.nodes(data=True):
        if data.get('label') == trend_text and data.get('type') == 'trend':
            return node_id
    return None
...
def _build_from_parquet(product_path: str, ip_path: str) -> nx.Graph:
    ...
    # 엣지 로딩: edgelist parquet 존재 시 직접 로드 (weight 반영)
    if os.path.exists(EDGELIST_PARQUET):
        print("[network_builder]   엣지: final_edgelist_with_trend.parquet 직접 로드")
        edge_df = pd.read_parquet(EDGELIST_PARQUET)
        _add_edges_from_edgelist(G, edge_df)
    else:
        print("[network_builder]   [경고] 엣지 파일이 없습니다. product/ip parquet에서 임시 재구성 시도 (일부 정보 누락 가능)")
        _add_edges(G, prod_df, ip_df)

    print(f"  빌드 완료: 노드 {G.number_of_nodes()}, 엣지 {G.number_of_edges()}")
    return G, prod_df, ip_df
...
def _add_nodes(G: nx.Graph, prod_df: pd.DataFrame, ip_df: pd.DataFrame):
    ...
    for _, row in prod_df.iterrows():
        G.add_node(
            ...
            label      = row['상품명'],
            type       = 'product',
            category   = row.get('중분류명', ''),   # 드롭다운 기준
            category_l = row.get('대분류명', ''),   # 대분류 (참고용)
            price      = row.get('인스타가격'),
    ...
    for _, row in prod_df.iterrows():
        all_attrs.update(_safe_list(row.get('최종키워드')))
    for ip_row in ip_df.itertuples():
        all_attrs.update(_safe_list(getattr(ip_row, '트렌드_속성', [])))
        all_attrs.update(_safe_list(getattr(ip_row, '일반_속성', [])))
    ...
    # 행사 노드
    if '행사명' in prod_df.columns:
        for ev in prod_df['행사명'].dropna().unique():
...
def _add_edges(G: nx.Graph, prod_df: pd.DataFrame, ip_df: pd.DataFrame):
    ...
    # post_dates 컬럼이 없거나 비어 있으면 기본값 1 사용
    ip_post_count = {
        row['ip_name']: max(len(_safe_list(row.get('post_dates'))), 1)
        for _, row in ip_df.iterrows()
    }
...
def _add_edges_from_edgelist(G: nx.Graph, edge_df: pd.DataFrame):
    """edgelist parquet 파일에서 직접 엣지를 로드 (가중치 반영)."""
    for _, row in edge_df.iterrows():
        src = str(row['src_node'])
        dst = str(row['dst_node'])
        w = row.get('weight', 1.0)
        edge_type = row.get('edge_type', 'unknown')
        
        # dst_type에 따라 노드 ID 포맷팅 (node 추가 방식과 일치시킴)
        src_formatted = src # product는 원본 ID 유지
        if row.get('src_type') == 'ip':
            src_formatted = f"ip_{src}"
            
        dst_formatted = dst
        if row.get('dst_type') == 'attr':
            dst_formatted = f"attr_{dst}"
        elif row.get('dst_type') == 'ip':
            dst_formatted = f"ip_{dst}"
            
        _add_or_update_edge(G, src_formatted, dst_formatted, w, edge_type=edge_type)
