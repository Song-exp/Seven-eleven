...
w('---')
w()
w('## 지표 요약 (상세 정의·공식 → `docs/keyword_classification_criteria.md` §1·§2)')
w()
w('표 읽기에 필요한 핵심만 recap — **전체 공식·근거는 위 기준서가 정본**:')
w('- **purity** = `Score_succ/(Score_succ+Score_fail)` — k로 흘러든 어텐션 중 성공제품 비중(어텐션 가중, 모델 관점). '
  '**기준선은 0.5가 아니라 base 0.238**(>0.238 성공특이 / <0.238 실패특이). 빈도 무관 비율값이라 키워드 간 동일 잣대 비교 가능.')
w('- **WoE** = `ln((supp_s/n_succ)/(supp_f/n_fail))` — purity의 **순수 카운트 버전**(통계 관점, 희소성 편향 X). '
  '기준선=0(>0 성공·<0 실패). killer는 `purity≥0.50` **AND** `WoE>0`를 둘 다 요구(어텐션 아티팩트 방어).')
w('- **Δprob**(개입·재추론) = 캐리어 고정+k 토글 시 성공확률 변화 = killer/mine/매개 확정의 인과 축.')
w('- **주의**: 빈도 1~2 소표본은 supp_f(또는 supp_s)=0이라 WoE가 ±13처럼 발산 → 절댓값 크다고 강한 신호 아님(빈도 floor·Wilson 하한이 거름).')
w()
w('## 표기 규약')
w()
